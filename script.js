document.addEventListener('DOMContentLoaded', ()=>{
    ...
    reset_btn.addEventListener('click', function(){
        ...
        // if (animationRunning) {
        //     pauseLoop.style.display = "block";
        //     playLoop.style.display = "none";
        // } else {
        //     pauseLoop.style.display = "none";
        //     playLoop.style.display = "block";
        // }
        ...
        // Reset select
        font_size_reset.value ="100"; // Reset font size
        // Reset loop duration
        loop_duration_reset.value="5"; 
        loop_duration_reset2.innerHTML="5"; 
        // Reset letter spacing 
        letter_spacing_reset.value = "5";
        letter_spacing_reset2.innerHTML= "5";
        // Reset speed
        speed_reset.value="1"; 
        // Reset select 
        ...
        cancelAnimationFrame(animationFrameId); // sithya
        clearCanvas();
        imageLoaded = false;
        currentImage = null;
    });
    ...
    //Lock and Unlock fullscreen function
    var lockScreen = document.getElementById("lockScreen");
    var unlockScreen = document.getElementById("unlockScreen");
    var fullScreenButton = document.getElementById("fullScreenButton");
    document.getElementById("lockScreenToggleBtn").addEventListener('click', function(){
        lockScreen.classList.toggle('active');
        unlockScreen.classList.toggle('active');
    });
    unlockScreen.addEventListener('click', function(){//masterly
        pauseLoopingBtn.style.display= "block";
    ...
    pauseLoopingBtn.addEventListener('click', function () {//sinat
        ...
        if (animationRunning) {
           ...
           // startTime = null; // Reset start time for new animation cycle
            animationFrameId = requestAnimationFrame(animateText); // Resume the animation
...
function clearCanvas() {
    ...
    c.scale(scale, scale);
    c.clearRect(0, 0, canvas.width, canvas.height); // Clear the canvas //sithya
}
...
function animateText(timestamp) {
    ...
    if (imageLoaded && currentImage) { //sithya
        const canvasAspect = canvas.width / canvas.height;
        ...
        if (imageAspect > canvasAspect) {
            ...
            sWidth = currentImage.height * canvasAspect;
            sx = (currentImage.width - sWidth) / 2;
            sy = 0;
        } else {
            sWidth = currentImage.width;
            sHeight = currentImage.width / canvasAspect;
            sx = 0;
...
document.getElementById("generate-btn").addEventListener("click", function () { //sithya
